...
# curl -fsSL https://raw.githubusercontent.com/KoDesigns/pretty-ls/main/install/install-linux.sh -o install.sh && bash install.sh
...
cleanup() {
    ...
    if [[ -n "${temp_file:-}" && -f "$temp_file" ]]; then
        rm -f "$temp_file"
        print_debug "Cleaned up temporary file"
    fi
    
    # Only show error message if we're actually exiting with an error
    if [[ $exit_code -ne 0 && "${BASH_SUBSHELL}" == "0" ]]; then
...
show_help() {
    cat << EOF
Pretty-ls (pls) - Linux Installation Script v${SCRIPT_VERSION}

This script installs the pretty-ls command line tool on Linux systems.
...
check_linux() {
    ...
    print_debug "Operating system: Linux ($(uname -r))"
}
...
check_curl() {
    if ! command -v curl >/dev/null 2>&1; then
        ...
        print_status "Install curl using your package manager:"
        print_status "  Ubuntu/Debian: sudo apt install curl"
        print_status "  CentOS/RHEL:   sudo yum install curl"
        print_status "  Fedora:        sudo dnf install curl"
...
check_dependencies() {
    ...
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        ...
        print_status "Install them using your package manager:"
        print_status "  Ubuntu/Debian: sudo apt install coreutils gawk grep sed"
        print_status "  CentOS/RHEL:   sudo yum install coreutils gawk grep sed"
        print_status "  Fedora:        sudo dnf install coreutils gawk grep sed"
        print_status "  Arch:          sudo pacman -S coreutils gawk grep sed"
        exit 1
    fi
...
download_script() {
    ...
    # Download with security and retry settings
    if ! curl -fsSL \
        --connect-timeout 30 \
        --retry 3 \
        --proto '=https' \
    ...
    print_debug "Script size: $(stat -c%s "$INSTALL_DIR/$SCRIPT_NAME" 2>/dev/null || echo "unknown") bytes"
}
...
main() {
    echo "🐧 Pretty-ls (pls) - Linux Installer v${SCRIPT_VERSION}"
    echo "========================================================"
    echo
